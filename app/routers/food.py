...
async def update_food(
    ...
    if image and image.filename:
        ...
        if db_food.food_image_url:
            # Extract filename from URL
            old_filename = db_food.food_image_url.split("/")[-1]
        ...
        base_url = str(request.base_url).replace("http://", "https://")
        update_data["food_image_url"] = f"{base_url}uploads/food/{unique_filename}"
    ...
    if update_data:
        ...
        # Update default variation if price changed
        if price is not None:
            default_variation = db.query(VariationOfFoodModel).filter(
                VariationOfFoodModel.food_id == food_id,
                VariationOfFoodModel.name == "For 1 person"
            ).first()

            if default_variation:
                default_variation.price = price
